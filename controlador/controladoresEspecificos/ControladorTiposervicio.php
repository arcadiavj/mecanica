...
class ControladorTiposervicio extends ControladorGeneral {        
    ...
    public function buscar() {//busca usando la clase SqlQuery
        $buscarTiposervicio = new SqlQuery(); //instancio la clase
        ...
        try {
            ...
            $statement = $this->refControladorPersistencia->ejecutarSentencia(
                $buscarTiposervicio->buscar($tabla)); //senencia armada desde la clase SqlQuery sirve para comenzar la busqueda
            $uno = $buscarTiposervicio->buscar($tabla);
            $arrayTiposervicio = $statement->fetchAll(PDO::FETCH_ASSOC); //retorna un array asociativo para no duplicar datos
            $this->refControladorPersistencia->get_conexion()->commit(); //si todo salió bien hace el commit
            return $arrayTiposervicio; //regreso el array para poder mostrar los datos en la vista... con Ajax... y dataTable de JavaScript
        } catch (PDOException $excepcionPDO) {
            echo "<br>Error PDO: " . $excepcionPDO->getTraceAsString() . '<br>';
            $this->refControladorPersistencia->get_conexion()->rollBack(); //si salio mal hace un rollback
        } catch (Exception $exc) {
    ...
    public function guardar($datosCampos) {//funcion guardar con SqlQuery implementado
        ...
        $guardar->verificarExistencia($tabla, $datosCampos["nTipo"])); //verifico si ya hay un usuario con ese nombre 
