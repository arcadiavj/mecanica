...
class ControladorSitrack extends ControladorGeneral {        

    

    public function buscar() {//busca usando la clase SqlQuery
        $buscarSitrack = new SqlQuery(); //instancio la clase
        (string) $tabla = get_class($this); //uso el nombre de la clase que debe coincidir con la BD           
        $campoSuma="kmrecorridos";
        $campoGrupo="Patente";        
        try {
            $this->refControladorPersistencia->get_conexion()->beginTransaction(); //comienza la transacción
            $statement = $this->refControladorPersistencia->ejecutarSentencia(
                $buscarSitrack->buscarSuma($tabla,$campoSuma,$campoGrupo )); //senencia armada desde la clase SqlQuery sirve para comenzar la busqueda
            $arraySitrack = $statement->fetchAll(PDO::FETCH_ASSOC); //retorna un array asociativo para no duplicar datos
            $this->refControladorPersistencia->get_conexion()->commit(); //si todo salió bien hace el commit
            return $arraySitrack; //regreso el array para poder mostrar los datos en la vista... con Ajax... y dataTable de JavaScript
    ...
    public function buscarJoin($datosCampo) {//busca usando la clase SqlQuery
        $buscarServicio = new SqlQuery(); //instancio la clase
        (string) $tabla = get_class($this); //uso el nombre de la clase que debe coincidir con la BD           
        try {
            $this->refControladorPersistencia->get_conexion()->beginTransaction(); //comienza la transacción
            $statement = $this->refControladorPersistencia->ejecutarSentencia(
                $buscarServicio->buscar($tabla,$datosCampo["id"],$datosCampo["nombreCampo"] )); //senencia armada desde la clase SqlQuery sirve para comenzar la busqueda
            $arrayServicio = $statement->fetchAll(PDO::FETCH_ASSOC); //retorna un array asociativo para no duplicar datos
            $this->refControladorPersistencia->get_conexion()->commit(); //si todo salió bien hace el commit
            return $arrayServicio; //regreso el array para poder mostrar los datos en la vista... con Ajax... y dataTable de JavaScript
        } catch (PDOException $excepcionPDO) {
            echo "<br>Error PDO: " . $excepcionPDO->getTraceAsString() . '<br>';
            $this->refControladorPersistencia->get_conexion()->rollBack(); //si salio mal hace un rollback
        } catch (Exception $exc) {
